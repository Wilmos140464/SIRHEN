...
STEP1 ()
### objet: Verification de la date de debut de la tache d'extraction TR18
###     Presence d un timestamp contenu dans la variable d'ordonnacement fic_tmstmp_tr18_extr_ok
{
	MESSAGE "### $FUNCNAME Recuperation de la date de debut de la tache d'extraction TR18 ${UXARCHIVE} ### "
	MESSAGE "Verification de l'initialisation de la variable fic_tmstmp_tr18_extr_ok"
	Command=' [ ! -z $fic_tmstmp_tr18_extr_ok ]'
        eval ${Command}
        TEST_ERROR ${?} "Variable fic_tmstmp_tr18_extr_ok non definie!"

	MESSAGE "Verification de la presence du drapeau d'ordonancement 032_tmstmp_tr18_extr_ok"
	Command=' [ -f $fic_tmstmp_tr18_extr_ok ] '
	eval ${Command}
	TEST_ERROR ${?} "Absence du drapeau d'ordonancement précisant la date de debut de l'extyraction TR18"

	MESSAGE	"Verification du contenu du drapeau d'ordonnacement"
	read -r line<$fic_tmstmp_tr18_extr_ok
	# le timestamp doit etre du genre 2015-03-11-21.22.20.000000
	Command=' [ ${#line} -eq 26 ]'
        eval ${Command}
        TEST_ERROR ${?} "Contenu errone du du drapeau d'ordonancement précisant la date de debut de  extraction TR18" 

	MESSAGE "###Verification du lancement du traitement"
	if [ "$verifdebut" = "true" ]; then
		verif_debut
	fi
}
...
### Connexion a la base TR18 et verification du nombre de traitements TR18_EXTR a l etat ETR
{
        ...
        declare -i wait_extr_tr18_ok=0
        [ $exec_scen_CDM_INFOAGT_REFE = "true" ] && wait_extr_tr18_ok=$((wait_extr_tr18_ok + 13))

        declare -i nb_extr_tr18_ok=0

        verify_end_TR18
        nb_extr_tr18_ok=$?
        MESSAGE "Debut d attente fin execution TR18 (au moins 2H)"
        MESSAGE "\tHoraire  fin estimee : $(date +%Y-%m-%d:%Hh%m.%S -d +2hours)"
        while [ $nb_extr_tr18_ok -ne $wait_extr_tr18_ok ]; do
                MESSAGE "\tOn attend 5 minutes avant de retester la fin du chargement TR18"
                sleep 300

                verify_end_TR18
                nb_extr_tr18_ok=$?
                MESSAGE "\t\t $nb_extr_tr18_ok termines / $wait_extr_tr18_ok attendus"
        done
...
###
{
	...
	mv ${rep_tr18_nfs}/sirhen* ${rep_tr18_nfs}/save/ 2>/dev/null
	if [ $? -ne 0 ]; then 
	...
	cp ${rep_sortie}/infocentre-affe/* ${rep_tr18_nfs} 2>/dev/null
	if [ $? -ne 0 ]; then
	...
	cp ${rep_sortie}/infocentre-car/* ${rep_tr18_nfs} 2>/dev/null
	if [ $? -ne 0 ]; then
                MESSAGE "\t Warning : aucun fichier a copier"
        else
                true
	...
	cp ${rep_sortie}/infocentre-fina/* ${rep_tr18_nfs} 2>/dev/null
        if [ $? -ne 0 ]; then
                MESSAGE "\t Warning : aucun fichier a copier"
        else
                true
	...
	cp ${rep_sortie}/infocentre-moye/* ${rep_tr18_nfs} 2>/dev/null
        if [ $? -ne 0 ]; then
                MESSAGE "\t Warning : aucun fichier a copier"
        else
                true
	...
	cp ${rep_sortie}/infocentre-papr/* ${rep_tr18_nfs} 2>/dev/null
        if [ $? -ne 0 ]; then
                MESSAGE "\t Warning : aucun fichier a copier"
        else
                true
	...
	cp ${rep_sortie}/infocentre-info/* ${rep_tr18_nfs} 2>/dev/null
        if [ $? -ne 0 ]; then
                MESSAGE "\t Warning : aucun fichier a copier"
        else
                true
	...
	cp ${rep_sortie}/infocentre-ref/* ${rep_tr18_nfs} 2>/dev/null
        if [ $? -ne 0 ]; then
                MESSAGE "\t Warning : aucun fichier a copier"
        else
                true
	...
	cp ${rep_sortie}/infocentre-sit/* ${rep_tr18_nfs} 2>/dev/null
        if [ $? -ne 0 ]; then
                MESSAGE "\t Warning : aucun fichier a copier"
        else
                true
	...
	MESSAGE "Copie des fichiers odi infocentre-gda sur le NFS"
        cp ${rep_sortie}/infocentre-gda/* ${rep_tr18_nfs} 2>/dev/null
        if [ $? -ne 0 ]; then
                MESSAGE "\t Warning : aucun fichier a copier"
        else
                true
...
###	Ajout temporaire en attendant que les scripts irhen utilisent le meme nfs
{
	...
	# tar l'arbo  les fichies de rep_sortie
        ...
        Command="mkdir tar_a_livrer"
        eval ${Command}

        Command="find info*/  \( -name '*.dat' -o -name '*.ctl' -o -name '*.ctr' \) -exec cp {} tar_a_livrer/ \;"
        eval ${Command}
	TEST_ERROR ${?} "Erreur: remplissage repertoire tar_a_livrer"

        Command="cd tar_a_livrer"
        eval ${Command}

	MESSAGE "Pause avant Generation du tar"
        Command="sleep 60"
	eval ${Command}
	...
	MESSAGE "Generation du tar"
        ...
        Command="tar czvf ${nom_tar_odi}.tar.gz . --exclude save --exclude annuaire-af --exclude annuaire-ldap --exclude ${nom_tar_odi}.tar.gz"
        eval ${Command}
#	TEST_ERROR ${?} "Erreur: creation du tar"

        MESSAGE ${?} "Erreur non trappee : retournée par la commande tar"

        Command="mv ${nom_tar_odi}.tar.gz ${rep_tar_odi}; cd .. ; rm -rf tar_a_livrer"
        eval ${Command}
	...
	MESSAGE "Copie vers le serveur SFTP"
        Command=" echo 'put ${rep_tar_odi}/${nom_tar_odi}.tar.gz ${rep_upload_odi}/depot_racine/' | sftp ${compte_upload_odi}@transfert.in.phm.education.gouv.fr "
        eval ${Command}
