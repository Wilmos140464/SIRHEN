...
###==========================================================================
#@(#) PROCEDURE: 	TR18
#@(#) OBJET: 		lancement des scenarios ODI SIRHEN pour LDAP
...
#set -x
	...
	db2 -x "update ex_odi.etattraitement set idetat= 'ATR' where idetat like 'A%' and idtypetraitement like '%_LDAP'"
	db2 -x "update ex_odi.etattraitement set idetat= 'ETR' where idetat like 'E%' and idtypetraitement like '%_LDAP'"
	
	nb_odi_encours=$(db2 -x "select count(idetat) from ex_odi.etattraitement where idetat like 'AEC%' and idtypetraitement like '%_LDAP' ")
...
###
{
        ...
        MESSAGE "Verification de l initialisation de la variable exec_scen_LDAP"
        [ ! -z $exec_scen_LDAP ]
        TEST_ERROR ${?} "Variable non initialisee!"

        ## lancement des scenarios SIRHEN TR18 LDAP

        if [ $exec_scen_LDAP = "true" ]; then
                MESSAGE "Suppression des anciens fichiers ldap archives ayant plus de 7 jours"
                find $rep_sortie/save/ldap/ -type f -ctime +7 -exec rm -f {} \;
                true
                TEST_ERROR ${?} "Erreur impossible"

                DATE=`date "+%Y%m%d%H%M"`

                MESSAGE "Archivage des fichiers du precedent traitement LDAP"
                [ ! -d $rep_sortie/save/ldap/ ] && mkdir $rep_sortie/save/ldap/
                ###mv $rep_sortie/annuaire-ldap/* $rep_sortie/save/ldap/ 2>/dev/null
                cp -r $rep_sortie/annuaire-ldap/* $rep_sortie/save/ldap/
                find $rep_sortie/annuaire-ldap/ -type f -name "*ldap*" -exec rm -f {} \;
                true
                TEST_ERROR ${?} "Erreur impossible"

                MESSAGE "Lancement scenario LDAP"
                touch $rep_declenchement_tr18/LDAP/Atraiter/sirhen_ldap_${DATE}.dsp
                TEST_ERROR ${?} "Creation du drapeau DSP pour le scenario LDAP impossible"
        fi
...
###
{
        ...
        ## Verification de la presence du fichier tmstmp_tr18_extr_ldap_ok. S'il n'y est pas, on le cree en y ajoutant un timestamp.
	
        Command="date +'%Y-%m-%d-%H.%M.%S.000000' > $fic_tmstmp_tr18_extr_ldap_ok; chmod 664 $fic_tmstmp_tr18_extr_ldap_ok"
        eval ${Command}
        TEST_ERROR ${?} "Impossible de creer le temoin ldap"
...
### objet: Lancement du scenario
        ...
        MESSAGE "scenario LDAP"
        Command="$start_scen_LDAP & "
        MESSAGE "Lancement de la commande : ${Command} "
        eval ${Command}
        TEST_ERROR ${?} "Erreur sur LDAP!"

#/appli/odi/oracledi/bin/startscen_QP3_TR18.sh TR18_ALM_LDAP 001 GLOBAL -NAME=AGENT_QP3_TR18 
