...
verif_debut ()
{
	...
	MESSAGE "tmstmp de lextraction TR18 est: $tmstmp"

	declare -i nb_debut=0
	while [ $nb_debut -eq 0 ]
  	do
    		sleep 10
		echo "la requete est : select count (IDETAT) from ex_odi.etattraitement where  IDETAT not like '%TR' and ts_update > '$tmstmp' and idtypetraitement not like '%_LDAP' and idtypetraitement not like '%_AAF'"
...
###     Presence d un timestamp contenu dans la variable d'ordonnacement fic_tmstmp_tr18_extr_ok
{
	...
	MESSAGE "Verification de la presence du drapeau d'ordonancement $fic_tmstmp_tr18_extr_ok"
...
STEP3 ()
### objet: Copie des fichiers de TR18 et generation du tar
### Une sauvegarde de la derniere execution est disponible sur ${rep_tar_odi}
{
        MESSAGE "### $FUNCNAME  Copie des fichiers et generation du tar ### "
...
STEP4 ()
### objet: Envoi du tar dans le sas de livraison
###
{
        MESSAGE "### $FUNCNAME Copie STP du tar et creation du flag### "
        MESSAGE "debut envoi tar : "
        Command="date"
        eval ${Command}

        MESSAGE "Copie vers le serveur SFTP du tar"
        ...
        eval ${Command}
        TEST_ERROR ${?} "Erreur: retournée par la commande put envoi du tar"
        MESSAGE "Fin copie vers le serveur SFTP :"
        Command="date"
        eval ${Command}

        MESSAGE "Copie vers le serveur SFTP du temoin"
        touch $mnt_irhen/TR18/ordonnanceur/${nom_flag_odi}_`date "+%Y%m%d"`
        Command=" ls -lrt $mnt_irhen/TR18/ordonnanceur"
        eval ${Command}
        MESSAGE "Envoi du flag de la Copie vers le serveur SFTP"
        Command=" echo 'put $mnt_irhen/TR18/ordonnanceur/${nom_flag_odi}_`date "+%Y%m%d"` ${rep_upload_odi}/depot_racine/' | sftp ${compte_upload_odi}@transfert.in.phm.education.gouv.fr "
        eval ${Command}
...
STEP5 ()
### objet: Compression des fichiers
###
{
        MESSAGE "### $FUNCNAME Compression des fichiers### "
