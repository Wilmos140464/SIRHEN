...
### 
{
	...
	if [ $exec_scen_LDAP = "true" ]; then
		MESSAGE "Suppression des anciens fichiers aaf archives ayant plus de 7 jours"
                ...
                cd $rep_sortie/save
                tar czvf ldap.tar.gz ldap/*
                sleep 5
                find $rep_sortie/save/ldap/ -type f -name "*ldap*" -exec rm -f {} \;
                rm -rf $rep_sortie/save/ldap
...
###
{
        ...
        ## Verification de la presence du fichier tmstmp_tr18_extr_ok. S'il n'y est pas, on le cree en y ajoutant un timestamp.
                date +'%Y-%m-%d-%H.%M.%S.000000' > $fic_tmstmp_tr18_extr_ldap_ok;
                chmod 664 $fic_tmstmp_tr18_extr_ldap_ok;
}
