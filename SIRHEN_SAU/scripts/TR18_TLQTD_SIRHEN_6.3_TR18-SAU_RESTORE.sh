...
#@(#) OBJET: 		Restauration de la base TR18 de SAU avec le backup TR18 de PR3
#@(#) AUTEUR: 		BG
#@(#) DATE CREATION: 	2016/12/31
#@(#)
###==========================================================================
...
echo -e "\n\nDÈbut de la restore de TR18 PR3 sur TR18 de $env : "`date`
      ...
      
{
	if [ ${1} != 0 ] ; then  
		...
		## Recuperation de la derniere log
	        trace=`ls -rt /mnt/applis_mid_sau/SIRHEN/logs/TR18_TLQTD_SIRHEN_6.3_TR18-SAU_RESTORE_db203_db2itr18*|tail -1`
       		 /bin/mail -s "sau-sir-db203.sirhen.hp.in.phm.education.gouv.fr : Plateforme SAU : Restore KO de la base TR18 SAU : `date` " sirhen-itp@education.gouv.fr < $trace
...

{
	...
	Command="source /mnt/applis_mid_$env/SIRHEN/conf/global_var_script_tr18"
	MESSAGE "Lancement de la commande : ${Command} "	
	eval ${Command}
	TEST_ERROR ${?} "Impossible de recuperer ou instancier les variables globales!"

	Command="source /mnt/applis_mid_$env/SIRHEN/conf/local_var_script_tr18_restore"
	MESSAGE "Lancement de la commande : ${Command} "	
	eval ${Command}
...
### objet: Verification du besoin de restauration du backup TR18 vers TR18 $env
###     Y-a-t-il une base TR18 : variable $exist_tr18 == true

{
	MESSAGE "### $FUNCNAME   Verification du besoin de restauration du backup TR18 vers TR18 de $env ${UXARCHIVE} ### "
	MESSAGE "Verification de l'initialisation de la variable exist_tr18"

	Command=' [ ! -z $exist_tr18 ] '
	eval ${Command}
	TEST_ERROR ${?} "Variable exist_tr18 non definie!"

	if [ "$exist_tr18" == false ]; then
		MESSAGE "Pas de base TR18 sur l'environnement $env => pas de backup"
		true
...
### objet: On verifie la presence de backup  TR18 sur le montage NFS /mnt/backup/GENERE_SUR_PR3
...
{ 
	...
	MESSAGE "Verification de la presence de backup TR18"
	eval ${Command}
#	TEST_ERROR ${?} "Aucun abcjkup TR18 sur le montage nfs"
}
...
### objet: On verifie si le dernier backup TR18 a deja ete consomme
###     Commentaire de l etape

{
        MESSAGE "### $FUNCNAME Verification de la consommation eventuelle du dernier backup TR18 ${UXARCHIVE} ### "
	MESSAGE "Verification de l'initialisation de la variable rep_bkp_sirhen"
        ...
        MESSAGE "Lancement de la commande : ${Command} "
#        eval ${Command}
#        TEST_ERROR ${?} "Le backup TR18 le plus recent a deja servi a restaurer TR18 de $env"
...
### objet: suppression des sauvegardes de TR18 archives en local
###     stockees sous $rep_rest

{
	MESSAGE "### $FUNCNAME  Suppression des sauvegardes de TR18 archivees en locale ${UXARCHIVE}  ### "
...
### objet: Suppression des logs de restauration anterieurs de la base TR18
###
...
### objet: recuperation du backup TR18
###

{
        ...
        eval ${Command}
        TEST_ERROR ${?} "Copie impossible du backup TR18 de $rep_bkp_sirhen vers $rep_rest"
}
...
### objet: Desactivation de la base TR18 pour restauration
###

{
        MESSAGE "### $FUNCNAME  Desactivation de la base TR18 $env pour restauration ${UXARCHIVE}  ### "
...

{
        ...
        TEST_ERROR ${?} "Generation impossible de la restore de la base $base_cible"
}
...
echo -e "\n\nFin du step13 restore sur SAU : "`date`
...

{
 ...
 ## Changer le logarchmeth1 (sinon, il conserve celui aui est embarqu‚?? dans la conf de la base source et cela g‚??n‚??re plein d'erreurs - inutiles - dans le db2diag)
	...
	echo "execution du script grant_uSUPERVISION_db2-9.x_v1.1.sh"
	echo "========================================================"
	./grant_uSUPERVISION_db2-9.x_v1.1.sh TR18 racdb2 poledb2
        echo "========================================================"
	echo "execution du script grant_specif_TR18.sh"
        echo "========================================================"
	./grant_specif_TR18.sh 
        echo "========================================================"
	db2 connect to $base_cible
...
echo -e "\n\nFin du step15 restore sur $env : "`date`
...
if [ ${CntStep} = $NbStep ]; then
        ...
        echo -e "\n\nFin de la restore OK de la base TR18  PR3 sur TR18 de  SAU : "`date`

	## Recuperation de la derniere log
        trace=`ls -rt /mnt/applis_mid_sau/SIRHEN/logs/TR18_TLQTD_SIRHEN_6.3_TR18-SAU_RESTORE_db203_db2itr18*|tail -1`
        /bin/mail -s "sau-sir-db203.sirhen.hp.in.phm.education.gouv.fr : Plateforme SAU : Fin de la restore OK de la base TR18 SAU : `date` " sirhen-itp@education.gouv.fr < $trace
