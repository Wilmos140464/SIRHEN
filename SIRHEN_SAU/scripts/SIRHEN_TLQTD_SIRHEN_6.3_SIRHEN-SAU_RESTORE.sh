...
#@(#) OBJET: 		Restauration de la base SIRHEN de SAU avec le backup SIRHEN de PR3
#@(#) AUTEUR: 		BG
...
echo -e "\n\nDÈbut de la restore de SIRHEN PR3 sur SAU : "`date`
...
###  	- $2: Message d'erreur (optionel)      
{
	if [ ${1} != 0 ] ; then  
		...
		## Recuperation de la derniere log
	        trace=`ls -rt /mnt/applis_mid_sau/SIRHEN/logs/SIRHEN_TLQTD_SIRHEN_6.3_SIRHEN-SAU_RESTORE_db201_db2isirh*|tail -1`
       		/bin/mail -s "sau-sir-db201.sirhen.hp.in.phm.education.gouv.fr : Plateforme SAU : Restore KO de la base SIRHEN PR3 sur SAU : `date` " sirhen-itp@education.gouv.fr < $trace
...
###	varibales globales statique et dynamique / variable locales statiques
{
	...
	Command="source /mnt/applis_mid_$env/SIRHEN/conf/global_var_script_sirhen"
	MESSAGE "Lancement de la commande : ${Command} "	
	eval ${Command}
	TEST_ERROR ${?} "Impossible de recuperer ou instancier les variables globales!"

	Command="source /mnt/applis_mid_$env/SIRHEN/conf/local_var_script_sirhen_restore"
	MESSAGE "Lancement de la commande : ${Command} "	
	eval ${Command}
...
### objet: Verification du besoin de restauration du backup sirhen vers SIRHEN SAU
###     Y-a-t-il une base SIRHEN : variable $exist_sirhen == true
{
	MESSAGE "### $FUNCNAME   Verification du besoin de restauration du backup sirhen vers SIRHEN SAU ${UXARCHIVE} ### "
	MESSAGE "Verification de l'initialisation de la variable exist_sirhen"
	Command=' [ ! -z $exist_sirhen ] '
	eval ${Command}
	TEST_ERROR ${?} "Variable exist_sirhen non definie!"

	if [ "$exist_sirhen" == false ]; then
		MESSAGE "Pas de base SIRHEN sur l'environnement SAU => pas de backup"
		true
...
{ 
	...
	eval ${Command}
	TEST_ERROR ${?} "Aucun backup SIRHEN sur le montage nfs"
}
...

{
	...
	MESSAGE "Verification de l'initialisation de la variable rep_bkp_sirhen"
        ...
        MESSAGE "Lancement de la commande : ${Command} "
        eval ${Command}
#        TEST_ERROR ${?} "Le backup SIRHEN le plus recent a deja servi a restaurer SIRHEN"
}
...
### objet: suppression des sauvegardes de SIRHEN archivees en local
...
### objet: Suppression des logs de restauration anterieurs de la base SIRHEN
###    
...
### objet: Desactivation de la base SIRHEN pour restauration
###
{
        MESSAGE "### $FUNCNAME  Desactivation de la base SIRHEN SAU pour restauration ${UXARCHIVE}  ### "
...

{
        ...
        TEST_ERROR ${?} "Generation impossible du script de restauration de la base $base_cible"
}
...

{
	...
	db2 terminate
        ...
        cd /db2data/db2isirh/shell/grants/
...
### objet: ANONYMISATION DE LA BASE SIRHEN SUR SAU
### modification de certaines tables de la base SAU
###  

{
        MESSAGE "### $FUNCNAME Operation d'anonymisation de la $base_cible ${UXARCHIVE} ### "

        db2 connect to $base_cible

        TEST_ERROR ${?} "connexion impossible a la base $base_cible"

        cd /mnt/applis_mid_$env/SIRHEN/Anonymisation/
        ./SIRHEN_AnonymisationMail.sh eiv@education.gouv.fr

        MESSAGE "Fin du passage de l'anonymisation sur la base $base_cible"

        MESSAGE ""
}

echo -e "\n\nFin du step16 restore sur SAU : "`date`
...
if [ ${CntStep} = $NbStep ]; then
        ...
        echo -e "\n\nFin de la restore OK de la base SIRHEN PR3 sur SAU : "`date`

	## Recuperation de la derniere log
        trace=`ls -rt /mnt/applis_mid_sau/SIRHEN/logs/SIRHEN_TLQTD_SIRHEN_6.3_SIRHEN-SAU_RESTORE_db201_db2isirh*|tail -1`
        /bin/mail -s "sau-sir-db201.sirhen.hp.in.phm.education.gouv.fr : Plateforme SAU : Fin de la restore OK de la base SIRHEN PR3 sur SAU : `date` " sirhen-itp@education.gouv.fr < $trace
